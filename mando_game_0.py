...
class Player(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if keys[pygame.K_UP] and self.y != 0 and (self.level[self.y - 1][self.x] == '%' or
                                                  self.level[self.y - 1][self.x] == 'O' or
                                                  self.level[self.y - 1][self.x] == ','):
            self.y -= 1
            self.rect.y -= tile_height
        if keys[pygame.K_DOWN] and self.y != len(self.level) - 1 and (self.level[self.y + 1][self.x] == 'O' or
                                                                      self.level[self.y + 1][self.x] == '%' or
                                                                      self.level[self.y + 1][self.x] == ',' or
                                                                      self.level[self.y + 1][self.x] == '.'):
            self.y += 1
            self.rect.y += tile_height
        if keys[pygame.K_LEFT] and self.x != 0 and (self.level[self.y][self.x - 1] == "." or
                                                    self.level[self.y][self.x - 1] == "," or
                                                    self.level[self.y][self.x - 1] == "P"):
            self.x -= 1
            self.rect.x -= tile_width
        if keys[pygame.K_RIGHT] and self.x != len(self.level[0]) - 1 and (self.level[self.y][self.x + 1] == "." or
                                                                          self.level[self.y][self.x + 1] == "," or
                                                                          self.level[self.y][self.x + 1] == "P"):
            self.x += 1
            self.rect.x += tile_width
